...
public class ObjectPoolRoad : MonoBehaviour
{
    ...
    {
        GameObject[] newObjects = new GameObject[objectsToPreload.Length + 1];
        for (int i = 0; i < objectsToPreload.Length; i++)
        {
            newObjects[i] = objectsToPreload[i];
        }
        newObjects[objectsToPreload.Length] = newObject;

        objectsToPreload = newObjects;
    }

    public void AddTimes(int newTimes)
    {
        int[] newTimesArray = new int[objectsToPreloadTimes.Length + 1];
        for (int i = 0; i < objectsToPreloadTimes.Length; i++)
        {
            newTimesArray[i] = objectsToPreloadTimes[i];
        }
        newTimesArray[objectsToPreloadTimes.Length] = newTimes;

        objectsToPreloadTimes = newTimesArray;
    }
    ...
    public void removeObjectsFromPool(GameObject sourceObject)
    {
        int uniqueId = sourceObject.GetInstanceID();
